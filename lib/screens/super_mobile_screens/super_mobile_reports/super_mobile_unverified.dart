...
class SuperMobileUnverified extends StatefulWidget {
  const SuperMobileUnverified({super.key});

  @override
  State<SuperMobileUnverified> createState() => _SuperMobileUnverifiedState();
}

class _SuperMobileUnverifiedState extends State<SuperMobileUnverified> {

  PrintingInfo? printingInfo;

  TextEditingController unverifiedStartController = TextEditingController();
  TextEditingController unverifiedEndController = TextEditingController();
  ...
  void initState() {
    super.initState();
    filteredStream = unverifiedData();
  }

  Stream<List<QueryDocumentSnapshot<Map<String, dynamic>>>> unverifiedData() {
        ...
        header: (context) => pw.Row(
            ...
            children: [
              ...
              pw.Column(
                  ...
                  children: [
                    pw.Text('Unverified', style: pw.TextStyle(font: bold, fontWeight: pw.FontWeight.bold, color: pdfBackgroundHeader)),
  ...
  Future<void> unverifiedStartDate(BuildContext context) async {
    ...
    if (picked != null) {
      setState(() {
        ...
        unverifiedStartController.text = DateFormat('yyyy-MM-dd').format(picked);
        if (kDebugMode) {
  ...
  Future<void> unverifiedEndDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      context: context,
    ...
    if (picked != null) {
      setState(() {
        ...
        unverifiedEndController.text = DateFormat('yyyy-MM-dd').format(picked);
        if (kDebugMode) {
  ...
  @override
  void dispose() {
    unverifiedStartController.dispose();
    unverifiedEndController.dispose();
    super.dispose();
  }
        ...
        body: NestedScrollView(
            headerSliverBuilder: (context, innerBoxIsScrolled) => [
              SliverAppBar(
                ...
                title: AppBarHeading1(text: 'Unverified'),
              )
            ],
                          ...
                          child: Row(
                            ...
                            children: [
                              Expanded(
                                  flex: 5,
                                  child: TextFormField(
                                    controller: unverifiedStartController,
                                    decoration: const InputDecoration(
                                      border: InputBorder.none,
                                    ),
                                    readOnly: true,
                                    onTap: () => unverifiedStartDate(context),
                                  )
                              ),
                              Expanded(
                                  child: IconButton(
                                    onPressed: () => unverifiedStartDate(context),
                                    icon: const Icon(
                        ...
                        child: Row(
                          ...
                          children: [
                            Expanded(
                                flex: 5,
                                child: TextFormField(
                                  controller: unverifiedEndController,
                                  decoration: const InputDecoration(
                                    border: InputBorder.none,
                                  ),
                                  readOnly: true,
                                  onTap: () => unverifiedEndDate(context),
                                )
                            ),
                            Expanded(
                                child: IconButton(
                                  onPressed: () => unverifiedEndDate(context),
                                  icon: const Icon(
                                    FluentSystemIcons.ic_fluent_calendar_date_filled,
                                    color: blue,
                                  ),
